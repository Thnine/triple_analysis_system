<template>
  <!-- <div class="container-fluid" style="width: 1920px;height: 1080px;position: relative;background-color: #999999;">
    <div class="view" style="float: left;position: absolute; top: 0px; left: 10px">
      <div id="my_dataviz" style="float: left;"></div>
      <svg id="svg0" width="700" height="303" style="float: left;"></svg>
    </div>
    <button @click="freshButton()">draw</button>
    <button @click="importDateButton()">import</button>
  </div> -->
    <div style="width: 1390px;height: 300px;border: solid;">
      <div class="view" style="float: left;position: absolute;left: 10px;">
        <div id="my_dataviz" style="float: left;">
          <div style="font-size: 30px;margin-left: -500px;">信息河流图</div>
          <!-- <RiverVieW style="width:650px;height:250px"></RiverVieW> -->
        </div>
        <div style="float: left;font-size: 30px;margin-left: 10px;">迁移气泡图</div>
        <svg id="svg0" width="700" height="270" style="margin-left: -150px;margin-top: -5px;"></svg>
      </div>
      <!-- <button @click="freshButton()" style="margin-left: 700px;">draw</button>
      <button @click="importDateButton()" style="margin-left: 700px;">import</button> -->
    </div>
</template>
...
import RiverVieW from "./RiverView.vue";
...
export default {
  ...
  methods: {
    ...
    Data2Stanard(test_data){
      let data = []
      
      for (let i = 0; i < test_data.length; ++i) {
        let data_line = test_data[i]
        let attr_list = this.attr_keys.map(key=>data_line[key])
        // let attr_list = [data_line["attr1"], data_line["attr2"], data_line["attr3"], data_line["attr4"], data_line["attr5"]]
        data.push({node: data_line["ip"], time: data_line["time"], type: attr_list.indexOf(Math.max(...attr_list)) + ""})
      }
      return data
    },

    drawStreamGraph(test_data) {

      const self = this;

      let data = this.Data2Stanard(test_data)

      this.time_list = this.getTimeList(data)
      let time_list = this.getTimeList(data)

      let new_data = []
      for (let i = 0; i < time_list.length; ++i) {
        // new_data.push({"attr0": 0, "attr1": 0, "attr2": 0, "attr3": 0, "attr4": 0, "year": time_list[i]})
        let tempData = {
          year:time_list[i]
        }
        for(let k of this.attr_keys){
          tempData[k] = 0
        }
      ...
      for (let i = 0; i < time_list.length; ++i) {
        for (let j = 0; j < data.length; ++j) {
          if (data[j]["time"] === time_list[i]) {
            // let type = "attr" + data[j]["type"]
            let type = self.attr_keys[parseInt(data[j]['type'])]
            new_data.find(item => item.year === time_list[i])[type] += 1
          }
        }
      }
      ...
      // let keys = ["attr0", "attr1", "attr2", "attr3", "attr4"]
      let keys = this.attr_keys
      ...
      for (let i = 0; i < data.length; ++i) {
        data[i]["year"] = (this.getTimeList(test_data).indexOf(data[i]["year"]) + "")
      }
      // Add X axis
      ...
      var area = d3.area()
        ...
        .y0(function(d) {return y(d[0]) - 150; })
        .y1(function(d) { return y(d[1]) - 150; })
    
      // Show the areas
      let std_data = this.Data2Stanard(test_data)
      let type_list = this.getTypeList(std_data)
      let poolInit = this.poolInit
      let drawBubbleChart = this.drawBubbleChart
      let changeSelect = this.changeSelect
      let getTypeSelect = this.getTypeSlect
      ...
      svg
        ...
        .attr("d", area)
        .on("click", function(d) {
          let type_select = type_list[d["index"]]
          type_select = type_select + ""
          if (type_select === getTypeSelect()) {
            type_select === getTypeSelect()
          } else {
            changeSelect(type_select)

            
            // self.drawBubbleChart(self.testData)
            ...
            poolInit(std_data, type_list, type_select)
    ...
    time2Integer(original_data) {
      let data = original_data
      var time_list = []
      for (var i = 0; i < data.length; ++i) {
        if (time_list.includes(data[i]["time"]) === false) {
          time_list.push(data[i]["time"])
        }
      }
      time_list.sort()
      for (var i = 0; i < data.length; ++i) {
        data[i]["time"] = time_list.indexOf(data[i]["time"])
      }
      return [data, time_list]
    },

    getTimeSlotNum(data) {
      return this.getTimeList(data).length
    },
    ...
    getInOutHopsDataForType(Data, type_select) {
      ...
      for (var t = 1; t < this.getTimeSlotNum(data); ++t) {
        for (var i = 0; i < type_list.length; ++i) {
          ...
          var row = i + 1
          var correlationIn = 0
          var correlationOut = 0
          for (var j = 0; j < data.length; ++j) {
            if (data[j]["type"] === type_name) {
              var node = data[j]["node"]
              if (data[j]["time"] === t) {
                 for (var z = 0; z < data.length; ++z) {
                  if (data[z]["node"] === node && data[z]["time"] === t-1 && data[z]["type"] === type_select) {
                    correlationIn += 1
                  }
                 }
              } else if (data[j]["time"] === t-1) {
                for (var z = 0; z < data.length; ++z) {
                  if (data[z]["node"] === node && data[z]["time"] === t && data[z]["type"] === type_select) {
                    correlationOut += 1
                  }
                 }
              }
    ...
    matrixTrans(data, type_select, type_list) {
      ...
      var hopColor = ["steelblue", "tomato", "#66c2a5"]
      d3.select("body").append("div").attr("class", "tip").style("display", "none")
      var margin = { top: 50, bottom: 10, left: 100, right: 30 }
    ...
    drawBubbleChart(data) {
      ...
      let getColorForType = this.getColorForType
      let poolInit = this.poolInit
