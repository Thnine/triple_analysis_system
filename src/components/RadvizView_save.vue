...
export default {
    ...
    methods:{
        ...
        processAttrData:function(){
            ...
            let odata = [{},{}]
            for(let i=0;i<this.newData.length;i++){
                for(let key in attr_nums){
                    if(this.attrName[key]==0)continue;
                    if(odata[0].hasOwnProperty(key)){
                        odata[0][key]+=this.newData[i]["attrs"][key]>0?1:0
                    }else{
                        odata[0][key]=this.newData[i]["attrs"][key]>0?1:0
                    }
                }
            ...
            for (let j in odata[0]) {
                pool2amount[j][0] += odata[0][j]/this.newData.length
                pool2amount[j][1] += odata[1][j]/this.newData.length
            }
            let attrs = []
            for (let key in pool2amount) {
                let temp = {}
                temp["pool"] = key
                temp["velocity"] = pool2amount[key][0]
                temp["range"] = pool2amount[key][1]
                attrs.push(temp)
            }
